...
equal to *h* the  detector adds *(s,h)* to state.
...
Communication between the  detector and a correct full node is reliable and bounded in time.


## Problem statement

The  detector gets as input a header at height *h* and
queries the secondaries for their headers. Eventually, the 
detector should
decide
...
*Remark:* By verifying we mean calling `VerifyHeaderAtHeight` from the
...
#### **[FN-LuckyCase]**:
The  full node on which the procedure is called remotely is correct and no timeout occurs at the caller on `Commit`.

#### **[FN-ManifestFaulty]**
The  full node on which the procedure is called remotely is faulty and a faulty header is returned.
...
```go
func ForkDetector(hd Header, trustedState TrustedState)  {
	for i, s range Secondaries {
		sh := Commit(s,hd.height)
		if validateSignedHeaderAndVals(sh,...) fails {
		    // validateSignedHeaderAndVals is defined in [verification]
