...
Tendermint failure model holds [TMBC-FM-2THIRDS]), and then define different
...
with *b.Header.Height + 1 = c.Header.Height*. We define the predicate
**signs(b,c)** to hold
    iff c.Header.LastCommit is in PossibleCommit(b).
	[TMBC-SOUND-DISTR-POSS-COMMIT.1](./verification#tmbc-sound-distr-poss-commit1).
	
#### **[TMBC-FUNC-SUPPORT.1]**


Let *b* and *c* be two light blocks. We define the predicate **supports(b,c,t)**
 to hold iff 
...
> That is, if the Tendermint failure model [TODO] holds, then c has
> been signed by at least on correct full node, cf.
> [[TMBC-VAL-CONTAINS-CORR.1]](TODO)
...
> the Tendermint Failure Model [TMBC-FM-2THIRDS] is violated, that is,
...
> The difference of the above definition to [TMBC-MC-FORK.1] is
> subtle. The latter requires a full node being affected by a bad
> block while [TMBC-BLOCKS-MCFORK.1] just requires that a bad block
> exists, possibly in memory of an attacker.

> The following captures a light client fork. There is no fork up to the height
> of block b. However, c is of that height, is different, and passes skipping
> verification. It is a stricter property than [TMBC-LC-ATTACK.1], as
> [TMBC-LC-ATTACK.1] requires that no correct full node is affected.
