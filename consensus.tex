...
\SPACE
\UPON{receiving $\li{\Proposal,height_p,round_p, proposal}$ \From\ $\coord(height_p,round_p)$ \With\ $step_p = \propose$} \label{line:tab:recvProposal} 
...
\IF{$height = height_p \wedge round = round_p \wedge step_p = \propose$}
	\STATE \PBroadcast \ $\li{\Prevote,height_p,round_p,\nil}$ to all 
...
\UPON{receiving $\li{\Prevote,height_p, round_p,*}$  \From\ at least $2f+1$ processes \With\
	$step_p \le \prevote$} \label{line:tab:recvAny2/3Prevote}
...
$step_p \le \prevote$} \label{line:tab:recvPrevote}
 \IF{$v \neq \nil$}
...
\IF{$height = height_p \wedge round = round_p \wedge step_p = \prevote$}
...
\SPACE
\UPON{receiving $\li{\Precommit,height_p,round,v}$ \From\ at least $2f+1$ processes \With\ $step_p < \commit$} \label{line:tab:onPrecommitRule}
  \IF{$v \neq \nil$}
...
\SPACE
\UPON{receiving $\li{\Precommit,height_p,round_p,*}$  \From\ at least $2f+1$ processes \With\ $step_p = \precommit$} \label{line:tab:startTimeoutPrecommit}
...
\IF{$height = height_p \wedge round = round_p \wedge step_p = \precommit$}
\STATE $StartRound(round_p+1)$
\ENDIF	
\ENDFUNCTION
